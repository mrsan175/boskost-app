...
export default function PropertyPerformance() {
  const { data: propertyData, error, isLoading } = useSWR("/api/dashboard/property-performance", fetcher);

  if (error) return <div className="text-red-500 text-xs">Gagal memuat performa properti</div>;
  ...
  if (!isLoading && (!propertyData || propertyData.length === 0)) {
    return (
      ...
      >
        <CardHeader>
          <CardTitle style={{ fontFamily: "var(--font-display)" }}>Performa Properti</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center justify-center py-12 gap-3 text-center">
            <HugeiconsIcon icon={Building01Icon} size={40} style={{ color: "var(--on-surface-variant)", opacity: 0.3 }} />
  ...
  return (
    ...
    >
      ...
      <CardContent className="space-y-4">
        {isLoading ? (
            [1, 2].map(i => <div key={i} className="h-24 bg-muted rounded-3xl" />)
        ) : (
          propertyData.map((p: any) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="font-bold text-sm" style={{ color: "var(--on-surface)" }}>{p.name}</p>
                  <p className="text-xs" style={{ color: "var(--on-surface-variant)" }}>
                    {p.total} Kamar{p.address ? ` • ${p.address}` : ""}
                  </p>
                  <Progress value={p.occupancyRate} className="mt-1.5 h-1 w-28" />
                </div>
              </div>
              <div className="flex gap-6 text-center">
                <div>
                  <p className="text-[10px] font-bold uppercase mb-1" style={{ color: "var(--on-surface-variant)" }}>Hunian</p>
                ...
                <div>
                  <p className="text-[10px] font-bold uppercase mb-1" style={{ color: "var(--on-surface-variant)" }}>Status</p>
