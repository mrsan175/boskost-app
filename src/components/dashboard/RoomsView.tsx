...
export function RoomsView({ initialRooms, propertyList }: RoomsViewProps) {
  ...
  const { data: allRooms = initialRooms } = useSWR("/api/dashboard/rooms", fetcher, {
    fallbackData: initialRooms,
    revalidateOnFocus: false,
  });

  // Deduplicate by ID just in case data comes in weird from SWR merge/fallback
  const uniqueRooms = Array.from(new Set(allRooms.map((r: any) => r.id)))
    .map(id => allRooms.find((r: any) => r.id === id));
  ...
  const grouped: Record<string, { propertyId: string; propertyName: string; roomList: any[] }> = {};
    ...
    <div style={{ background: "transparent" }}>
      ...
      {groups.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          ...
          <p className="text-lg font-bold opacity-60">Tidak ada kamar yang sesuai filter.</p>
        </div>
      ) : (
        groups.map((group) => (
          <div key={group.propertyId} className="mb-10 space-y-6">
            <div className="flex items-center gap-2">
              <div className="h-5 w-1 rounded-full" style={{ background: "var(--primary)" }} />
              <h3 className="text-lg font-extrabold" style={{ fontFamily: "var(--font-display)" }}>
                {group.propertyName}
              </h3>
              <span className="text-xs opacity-50 font-bold">({group.roomList.length} Kamar)</span>
            </div>
