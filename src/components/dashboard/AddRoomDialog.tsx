...
export function AddRoomDialog({ propertyId, limitReached, properties }: AddRoomDialogProps) {
  const [open, setOpen] = useState(false);
  const [isPending, startTransition] = useTransition();
  const [error, setError] = useState<string | null>(null);
  const [isBulk, setIsBulk] = useState(false);
  const [priceValue, setPriceValue] = useState<string>("");
  const [selectedPropertyId, setSelectedPropertyId] = useState(propertyId === "all" ? "" : propertyId);
          ...
          <DialogHeader className="p-6 pb-2 shrink-0">
            <div className="flex items-center gap-3 mb-1">
              ...
              <div className="flex-1">
                ...
                <p className="text-[10px] opacity-60 uppercase tracking-widest font-bold">Input data ruangan properti Anda</p>
              </div>
            </div>
            <DialogDescription>
              {limitReached 
            ...
            {limitReached ? (
              <div className="flex-1 overflow-y-auto px-6 space-y-6 py-10 flex flex-col justify-center text-center">
                ...
                >
                  ...
                  <div>
                    <h4 className="font-bold text-2xl" style={{ color: "var(--on-surface)", fontFamily: "var(--font-display)" }}>Batas Maksimal</h4>
                    <p className="text-sm mt-2 leading-relaxed opacity-60">
                      Anda telah mencapai batas maksimal kamar untuk akun FREE. Upgrade ke PRO untuk menambah kamar tanpa batasan!
                    </p>
                  </div>
                  <Button 
                ...
                <div className="pt-4">
                  <Button variant="outline" onClick={() => setOpen(false)} className="rounded-full h-11 border-outline-variant">Tutup</Button>
                </div>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
                {/* Mode Toggler - Sticky/Fixed at top */}
                <div className="px-6 py-4 border-b shrink-0 bg-surface/50 backdrop-blur-md" style={{ borderColor: "var(--outline-variant)" }}>
                  <div 
                ...
                <div className="flex-1 overflow-y-auto px-6 space-y-4 py-6 custom-scrollbar">
                  {propertyId === "all" && properties && (
                    <div className="space-y-1.5">
                      <Label htmlFor="propertySelect">Pilih Properti <span className="text-red-500">*</span></Label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className={isBulk ? "col-span-2 space-y-1.5" : "space-y-1.5"}>
                      ...
                      {isBulk && <p className="text-[10px] font-bold opacity-60 text-primary ml-1">💡 Harga & Catatan akan disamakan ke semua nomor ini.</p>}
                    </div>
                    
                    {!isBulk && (
                      <div className="space-y-1.5">
                        <Label htmlFor="floor">Lantai</Label>
                        <Input id="floor" name="floor" type="number" min="1" defaultValue="1" disabled={isPending} />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="notes">Catatan Fasilitas {isBulk ? "(Dipakai bersama)" : ""}</Label>
                    <Textarea 
                      id="notes" 
                      name="notes" 
                ...
                <DialogFooter className="p-6 border-t shrink-0 gap-3 bg-surface" style={{ borderColor: "var(--outline-variant)" }}>
                  <Button type="button" variant="outline" onClick={() => setOpen(false)} disabled={isPending} className="rounded-full">Batal</Button>
                  <Button 
                    type="submit" 
                    disabled={isPending} 
                    ...
                    style={{ background: "var(--gradient-cta)" }}
                  >
                    {isPending ? "Menyimpan..." : isBulk ? `Simpan Semua` : "Simpan Kamar"}
                  </Button>
                </DialogFooter>
              </form>
            )}
